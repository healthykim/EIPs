...
**New message type `Cells` (`0x13`).** Used to respond to a `GetCells` requests. A peer MAY omit an entire transaction or individual indices if they are unavailable or if the response exceeds the packet size limit.
