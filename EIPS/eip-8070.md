...
During the transition, nodes will have both `eth/71` and `eth/72` peers.

* A type 3 transaction may only be announced to an `eth/71` peer when the node has the full blob payload, since the peer may request the complete sidecar.

* Type 3 transactions received from `eth/71` peers include the full blob payload and may be re-announced to `eth/72` peers with an all-ones `cell_mask`.
