...
We introduce a new devp2p protocol version, `eth/71` extending the existing [`eth/70`](https://eips.ethereum.org/EIPS/eip-7975) protocol.
...
**New message type `Cells` (`0x13`).** Used to respond to a `GetCells` requests. A peer MAY omit an entire transaction if they are unavailable or if the response exceeds the packet size limit. However, it MUST include all columns specified in the cell_mask field of the request in its response.
...
Please note that after the announcement, the node MUST try its best to serve the transaction for a certain period. In addition, intentionally sending only part of the columns to force neighbor providers to reconstruct full blob in this period is prohibited (see "Reconstruction" and "Propagation and Retention Phase" sections in the Security Considerations).

**Sampler role.**
If the node is a sampler, the node MUST only request the transaction payload via `GetPooledTransactions`. It SHOULD await to observe at least 2 distinct provider announcements, and to successfully receive and validate the signed transaction, before acting further. It then MUST request custody-aligned cells from peers that announced overlapping availability, including providers. When fetching from a provider, the node MAY request extra random columns in addition to its custody set (see "Sampling noise" in Security Considerations).

Likewise, after collecting and validating all required cells, the sampler MUST announce the transaction hash to its peers via NewPooledTransactionHashes. In this announcement, it MUST specify in the cell_mask field the cells it can serve.

Over time, peers may drop data or be disconnected, which can cause availability information to become outdated. To prevent this, the node SHOULD perform retesting on blobs that were verified long ago. Retesting means resampling columns that have not been checked for a certain period to update availability information. To reduce overhead, only a subset of transactions in the blobpool SHOULD be retested selectively. The sampling frequency can vary by client implementation, and upcoming proposers are expected to perform it more frequently.

Each node MAY maintain a local "World View" structure to maintain availability information. This internal structure tracks which peers appear to hold which blob cells. It can be represented as a matrix where rows correspond to peer IDs and columns to blob columns, with each entry indicating whether the peer is believed to hold the data. This data structure can be updated by retesting or announcements from peers. For example, provider announcements update full rows, while sampler announcements update specific cells for custody columns. Each record includes a timestamp that decays over time, lowering confidence in older data. When confidence falls below a certain threshold, the node SHOULD schedule retesting for that entry.

Retesting SHOULD take place after the transaction has been stored in the mempool for longer period, rather than during its initial propagation (see "Propagation and Retention Phase" section in Security Considerations).
...
Custody-aligned sampling generates stable and predictable request patterns that attackers could game. For example, a malicious node may trick a victim by pretending to be a provider when in reality it only stores the few, predictable columns the victim is expected to sample. To defend against this, this EIP suggests the simple mechanism of "sampling noise."

When a peer requests its custody columns from a provider, it may also request at least ONE randomly selected column. A failure to serve this extra column can be interpreted as a strong sign of misbehavior. The attacker could choose to timeout, but after a number of repetitions, the victim would likely disconnect anyway due to high failure rates. This mechanism can reduce the size of the victim peerset that such attacker can target, or increases the attacker’s cost to sustain such an attack.

While random sampling by itself is not sufficient to fully defend against data withholding or malicious providers who hold full blob, it can work in conjunction with other mechanisms, such as maintaining a minimum threshold of providers, to strengthen overall security. 

It also prevents malicious providers from persistently poisoning peers, since the added randomness disrupts any static mapping that attackers could otherwise exploit indefinitely. This cumulative randomness across the network could make prediction increasingly difficult over time.
...
### Propagation Phase and Retention Phase

The transaction propagation process is divided into two phases, and the node’s responsibilities differ depending on each phase.

1. **Propagation Phase**: This phase represents the initial stage when a transaction is first propagated through the network, typically lasting from a few seconds to a few minutes. Nodes should not immediately drop peers due to a lack of response, but they are expected to make their best effort to respond to requests related to transactions during this period.

2. **Retention Phase**: This phase covers the long-term period during which a transaction remains in the mempool, awaiting inclusion in a block. During this stage, nodes should focus on maintaining transaction availability. It is recommended that nodes make eviction decisions based on multiple factors such as the success of retesting, the availability signals from neighboring peers, the transaction’s age, and its economic value.

### Reconstruction

Reconstruction is prohibitive during the propagation phase, and nodes must not send only a portion of columns to force peers to reconstruct the rest. If an attacker transmits only 51% of the total columns by default and lets the network reconstruct the remaining part, the attacker can impose excessive CPU load across the network while consuming minimal resources themselves. This selfish behavior creates asymmetric resource consumption and shifts transmission costs from the sender to other nodes. Furthermore, allowing such behavior would incentivize rational nodes to transmit only 50% and a small surplus of the columns and rely on the network to handle the rest. This form of lazy publishing undermines both propagation efficiency and fairness.

However, when a transaction in the retention phase is considered economically valuable, it may be reasonable to spend some CPU resources to reconstruct its blobs in order to maintain overall availability. The resource cost of reconstruction can also be amortized over the long retention period in this case.
