...
author: Raúl Kripalani (@raulk), Bosul Mun (@healthykim), Francesco D'Amato (@fradamt), Csaba Kiraly (@cskiraly), Felix Lange (@fjl), Marios Ioannou (@mariosioannou-create), Alex Stokes (@ralexstokes), Kamil Salakhiev (@kamilsa)
...
**Modify `Transactions` (`0x02`) behaviour.**

[EIP-4844](./eip-4844.md) prohibits the direct broadcast of blob transactions:
...
The above should be changed as follows:
    ...
    Nodes MAY eagerly broadcast blob transaction **without** blob payloads as in `PooledTransactions` responses to their peers. Peers can then request cells using `GetCells` messages. Nodes MUST NOT forward a type 3 transaction before it has received and validated the transaction and the cells corresponding to its role (full payload for providers, custody cells for samplers).
...
Upon receiving a type 3 transaction that replaces a transaction that already exists in blobpool, with the same sender and nonce, nodes MAY reuse the cells that have already been fetched for the previous transaction if their `blob_versioned_hashes` are identical. The node MAY also immediately re-announce the replacement with the same `cell_mask`.
...
This EIP extends `engine_forkchoiceUpdatedV4` as defined in the Amsterdam Engine API specification by adding an optional `custodyColumns` parameter, allowing the CL to inform the EL of its current blob column custody set.
...
### Replacement transactions and direct propagation

A typical blob transaction replacement changes only the fee fields. It is also used most in periods of fee volatility, and a network overload is the typical case of such a situation. Thus, when there is already high demand, we make the situation worse by adding extra traffic redistributing blob content that was already distributed.

Because cell retrieval is separate from transaction retrieval, a node can reuse cells from the previous transaction when the replacement has the same `blob_versioned_hashes`. Note that reuse applies to the full set of cells. `GetCells` is keyed by transaction hash, not by blob content, so partial reuse would require additional protocol machinery. Commitments and cell proofs are still sent with the transaction payload.

This optimization is similar to [EIP-8094](./eip-8094.md), which aims to avoid additional blob bandwidth for replacement transactions.
